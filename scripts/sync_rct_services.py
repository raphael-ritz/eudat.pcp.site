#
# sync_rct_services.py
#
# simple script to be invoked via
#
#  cd <your-buildout-root-dir>
#  bin/instance run scripts/sync_rct_services.py [site_id]
#
...
rct_services = rct_data['rct.service'].copy()
...
for pk, values in rct_services.items():
    ...
    if id not in targetfolder.objectIds():
        ...
        print "Added %s to the services folder" % id
        
    data = preparedata(values)
    print data
    targetfolder[id].edit(**data)
    targetfolder[id].reindexObject()
    print "Updated %s in the services folder" % id
